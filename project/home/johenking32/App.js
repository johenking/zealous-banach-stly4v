...
try {
  ...
  if (configStatus === "valid" || configStatus === "canvas") {
    ...
    // 尝试开启离线持久化 (Firebase Native Offline Mode)
    // 注意：在某些隐私模式或 iframe 环境中可能会失败，所以需要 try-catch
    try {
      enableIndexedDbPersistence(db).catch((err) => {
        if (err.code === "failed-precondition") {
          console.warn("多标签页打开导致离线持久化失败");
...
const CURRICULUM_DATA = {
  ...
  3: [
    ...
    { name: "大学英语(3)", credits: 2, category: "通识必修" },
  ...
  4: [
    ...
    { name: "大学英语(4)", credits: 2, category: "通识必修" },
...
// =================================================================
// 🎨 组件：骨架屏 (Shimmer Effect)
// =================================================================
const SkeletonCard = () => (
  <div className="bg-white rounded-2xl p-5 border border-slate-100 relative overflow-hidden h-32">
    <div
      className="animate-shimmer absolute inset-0 bg-gradient-to-r from-transparent via-slate-100/50 to-transparent -translate-x-full"
      style={{ zIndex: 1 }}
    ></div>
...
export default function App() {
  ...
  // --- Data Sync (Hybrid: Cache + Firestore) ---
  useEffect(() => {
    ...
    // 如果有缓存，可以提前结束 loading 状态，让用户先看
    if (cachedPlan || cachedElec) setLoading(false);

    // 步骤 B: 建立 Firestore 监听 (后台静默更新)
    const planRef =
      configStatus === "canvas"
        ? doc(
            db,
    ...
    const unsubPlan = onSnapshot(
      ...
      (docSnap) => {
        ...
        saveToCache(user.uid, "planProgress", data); // 更新缓存
        setLoading(false);
      },
      (err) => {
        console.warn("Firestore error (possibly offline):", err);
        setLoading(false);
      }
    );

    const unsubElec = onSnapshot(
      query(electivesRef),
      (snap) => {
        const data = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        setElectives(data);
        saveToCache(user.uid, "electives", data); // 更新缓存
        setLoading(false);
      },
      (err) => {
        console.warn("Firestore error (possibly offline):", err);
  ...
  const handleToggleFixedCourse = async (courseName) => {
    ...
    setPlanProgress(newData); // 立即更新 UI
    saveToCache(user.uid, "planProgress", newData); // 立即更新缓存

    try {
      await setDoc(planDoc, newData);
    } catch (e) {
      console.error(
        "Sync failed, will retry automatically if persistence enabled",
        e
      );
    }
  };
  ...
  const handleAddElective = async (status) => {
    ...
    // 对于 List 类型的乐观更新稍微复杂，这里暂时只依赖 Firebase 的本地队列
    // 因为添加操作不如 Toggle 操作那么高频
  ...
  // 1. Loading Screen (Optimized Visual)
  if (authLoading)
    return (
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-slate-50 font-sans p-6">
        <div className="bg-white p-10 rounded-3xl shadow-xl max-w-lg w-full text-center">
          ...
          <h1 className="text-xl font-bold text-slate-800 mb-2">配置未完成</h1>
          <p className="text-slate-500">请在代码中填入 Firebase API Key。</p>
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 to-indigo-50/50 p-6 font-sans">
        <div className="bg-white/80 backdrop-blur-xl p-8 md:p-12 rounded-[2.5rem] shadow-xl shadow-indigo-100 max-w-md w-full text-center border border-white">
          ...
          <button
            ...
            className="w-full bg-slate-900 text-white py-4 px-6 rounded-2xl font-bold text-base hover:bg-slate-800 transition-all flex items-center justify-center shadow-xl mb-4 tap-active"
          >
            使用 Google 账号登录
          </button>
          <button
            onClick={handleGuestLogin}
            className="w-full bg-slate-100 text-slate-600 py-4 rounded-2xl font-bold text-sm hover:bg-slate-200 transition-all flex items-center justify-center tap-active"
          >
            我是访客 (直接试用)
          </button>
        </div>
  ...
  // 计算标签栏的背景位置 (简单的平滑切换逻辑)
  const tabIndex = ["plan", "electives", "stats"].indexOf(activeTab);
  const tabWidth = 100 / 3;

  return (
    <div
      className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans pb-24 md:pb-10 selection:bg-indigo-100 overscroll-y-none"
      ref={topRef}
    >
      {/* 顶部导航 (Glassmorphism & Sticky) */}
          ...
          <div className="flex items-center gap-3">
            ...
            {isOnline && (
              <div className="hidden md:flex items-center text-green-600 bg-green-50 px-2 py-1 rounded-md border border-green-100">
                ...
                <span className="text-[10px] md:text-xs font-bold">已连接</span>
              </div>
            )}

            <span className="hidden md:inline text-xs font-mono text-slate-400 bg-slate-100 px-2 py-1 rounded-md">
              {user.displayName || "User"}
            </span>
            <button
              onClick={() => signOut(auth)}
              className="p-2 hover:bg-red-50 hover:text-red-500 rounded-full transition-colors tap-active"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 mt-4 md:mt-8 space-y-6">
          ...
          <div className="bg-slate-200/60 p-1 rounded-xl relative flex h-11 backdrop-blur-sm">
            ...
            {["plan", "electives", "stats"].map((tab) => (
              <button
                ...
                className={`flex-1 relative z-10 text-xs font-bold transition-colors duration-200 ${
        ...
        <div className="hidden md:flex bg-white p-1.5 rounded-2xl shadow-sm inline-flex mb-6 border border-slate-100">
          {["plan", "electives", "stats"].map((tab) => (
            ...
            >
              ...
              {
                { plan: "我的课表", electives: "课程录入", stats: "学分统计" }[
                  tab
                ]
              }
            </button>
            ...
            {!loading &&
              [1, 2, 3, 4, 5, 6, 7, 8].map((sem) => {
                ...
                const progress = Math.round(
                  (completed / realCourses.length) * 100
                );
                const planWidth = Math.round(
                  ((completed + planned) / realCourses.length) * 100
                );
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full bg-slate-100 h-2 rounded-full mb-5 overflow-hidden relative">
                      ...
                      <div
                        className="absolute left-0 top-0 bg-amber-300 h-full transition-all duration-1000 ease-out"
                        style={{ width: `${planWidth}%` }}
                      ></div>
                      <div
                        className="absolute left-0 top-0 bg-indigo-500 h-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(99,102,241,0.5)]"
                      ...
                      {semCourses.slice(0, 3).map(
                        (c) =>
                          !c.isPlaceholder && (
                            ...
                            >
                              <div
                                className={`w-2 h-2 rounded-full mr-2.5 flex-shrink-0 transition-colors duration-300 ${
                                  planProgress[c.name]?.status === "completed"
                                    ? "bg-green-400"
                                    : planProgress[c.name]?.status === "planned"
                                    ? "bg-amber-400"
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <div className="relative">
                      <select
                        className="w-full bg-slate-50 border-none rounded-xl px-4 py-3.5 text-sm font-bold focus:ring-2 focus:ring-indigo-500 appearance-none text-slate-700"
                  ...
                  <div>
                    ...
                    <input
                      className="w-full bg-slate-50 border-none rounded-xl px-4 py-3.5 text-sm focus:ring-2 focus:ring-indigo-500 text-slate-700 font-medium"
                      placeholder="例如：插画设计"
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <button
                      ...
                      className="bg-amber-50 text-amber-700 hover:bg-amber-100 py-3.5 rounded-xl text-sm font-bold transition-all tap-active flex items-center justify-center"
                    >
                      <Clock className="w-4 h-4 mr-1.5" /> 计划
                    </button>
                    <button
                      onClick={() => handleAddElective("completed")}
                      className="bg-indigo-600 text-white hover:bg-indigo-700 py-3.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-indigo-200 tap-active flex items-center justify-center"
                    >
            ...
            <div className="md:col-span-2 space-y-3 pb-20">
              {electives.length === 0 ? (
                <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
                  <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">
                ...
                electives.map((item) => (
                  <div
                    key={item.id}
                    className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center group tap-active animate-in slide-in-from-bottom-2 duration-300"
                  >
          ...
          <div className="animate-in fade-in zoom-in-95 duration-300 space-y-8 md:space-y-10 pb-20">
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[2rem] p-6 md:p-8 text-white shadow-xl shadow-indigo-200 flex flex-col md:flex-row items-center justify-between relative overflow-hidden">
              ...
              <div className="flex items-center space-x-6 relative z-10 w-full md:w-auto mb-6 md:mb-0">
                <div className="p-4 bg-white/10 backdrop-blur-md rounded-2xl hidden md:block border border-white/10">
                ...
                <div className="text-center md:text-left w-full md:w-auto">
                  ...
                  <div className="mt-2 text-sm bg-white/20 inline-block px-3 py-1 rounded-lg backdrop-blur-md">
                    GPA:{" "}
                    <span className="font-bold text-white">{stats.gpa}</span>
                  </div>
                </div>
              </div>
              <div className="relative z-10 w-full md:w-auto">
                <div className="flex items-center justify-between md:justify-end gap-3 bg-white/10 px-5 py-4 rounded-xl border border-white/10 backdrop-blur-md">
                  <span className="text-xs font-bold text-amber-300 flex items-center">
                    <Clock className="w-4 h-4 mr-2" /> 计划中
                  </span>
            ...
            <section>
              <div className="flex items-center space-x-2 mb-4 px-1">
                ...
                <h3 className="font-bold text-slate-800">通识核心 (2分/项)</h3>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center text-white shadow-sm ${config.color} ${config.shadow}`}
                        >
                          {isDone ? (
                            <CheckCircle2 className="w-5 h-5" />
                          ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                <div className="flex items-center space-x-2 mb-5">
                  ...
                  <h3 className="font-bold text-slate-800">拓展培养</h3>
                </div>
                <div className="space-y-5">
                  {Object.keys(EXTENSION_MODULES).map((cat) => {
                    const data = stats.breakdown[cat];
                    const progress = Math.min(
                      (data.earned / data.required) * 100,
                      100
                    );
                    return (
                      <div key={cat}>
                        ...
                        <div className="h-2 bg-slate-50 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-1000 ease-out ${EXTENSION_MODULES[cat].color}`}
                            style={{ width: `${progress}%` }}
                          />
                        </div>
                      </div>
              ...
              <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                <div className="flex items-center space-x-2 mb-5">
                  ...
                  <h3 className="font-bold text-slate-800">创新创业</h3>
                </div>
                <div className="space-y-5">
                  {Object.keys(INNOVATION_MODULES).map((cat) => {
                    ...
                    return (
                      <div key={cat}>
                        ...
                        <div className="h-2 bg-slate-50 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-1000 ease-out ${INNOVATION_MODULES[cat].color}`}
                            style={{ width: `${progress}%` }}
                          />
                        </div>
                      </div>
      ...
      {/* 学期详情弹窗 (Mobile: Bottom Sheet, Desktop: Right Drawer) */}
      {selectedSemester && (
        <div className="fixed inset-0 z-50 flex items-end md:items-stretch md:justify-end">
          ...
          <div className="relative bg-[#f8fafc] w-full md:max-w-md h-[85vh] md:h-full shadow-2xl rounded-t-[2rem] md:rounded-l-[2rem] md:rounded-tr-none flex flex-col modal-enter overflow-hidden">
            ...
            <div className="px-6 md:px-8 py-4 flex justify-between items-center bg-white border-b border-slate-100 z-10">
              ...
              <button
                onClick={() => setSelectedSemester(null)}
                className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-all tap-active"
              >
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <button
                        ...
                        className={`mt-1 w-6 h-6 rounded-full flex items-center justify-center border-2 transition-all flex-shrink-0 tap-active ${
                          status === "completed"
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start">
                          <h4
                            className={`font-bold leading-snug text-sm transition-all ${
                              status === "completed"
                                ? "text-green-800 line-through opacity-50"
                                : "text-slate-700"
                            }`}
                          >
                            {course.name}
                          ...
                          <span
                            className={`text-[10px] text-white px-1.5 py-0.5 rounded font-bold shadow-sm ${
